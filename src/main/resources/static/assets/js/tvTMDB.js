$(document).ready(function() {
	...
	function getMovies() {
		showLoadingMessage("#tv-shows", "Loading TV shows...");
	...
	function loadTmdbShows(climateMovies) {
		let sort = sessionStorage.getItem("Sort");
		if (sort == null) sort = "";
		let Genre = sessionStorage.getItem("Genre");
		if (Genre == null) Genre = "";
	...
	function appendTmdbTvCard($container, respElement, climateMovies) {
		...
		const image = createImageElement(posterUrl, title ? "Poster for " + title : "TV show poster", placeholderOptions);
		if (image) {
