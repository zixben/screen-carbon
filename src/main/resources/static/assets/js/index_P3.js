$(document).ready(function () {
    ...
    const MAX_REQUESTS_PER_MINUTE = 30; // Adjust to TMDB's rate limit
    const INTERVAL = 60000 / MAX_REQUESTS_PER_MINUTE;
    let requestQueue = [];
    ...
    function scheduleRequest(fn) {
        return new Promise((resolve) => {
            requestQueue.push(() => fn().then(resolve));
    ...
    function fetchAndRenderTopRanked() {
        fetchScoreData().then((response) => {
            ...
            let requests = response.map((item) => scheduleRequest(() => getPopularity(item)));
