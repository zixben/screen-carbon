	...
	function renderVideos(videos) {
		...
		videos.forEach(function(video) {
			...
			const placeholderOptions = {
				...
				placeholderIcon: "bi bi-tv",
				placeholderClassName: "media-placeholder--poster media-placeholder--compact"
			};
			const image = createImageElement(fallbackPosterUrl, title ? "Poster for " + title : "TV show poster", placeholderOptions);
			if (image) {
				$imageWrapper.append(image);
			} else {
				$imageWrapper.append(createImagePlaceholder(placeholderOptions.placeholderLabel, placeholderOptions));
			}
			resolveTmdbPosterUrl("tv", videoId, video.vImg).then(function(resolvedPosterUrl) {
				if (resolvedPosterUrl && resolvedPosterUrl !== fallbackPosterUrl) {
					setImageContent($imageWrapper.get(0), resolvedPosterUrl, title ? "Poster for " + title : "TV show poster", placeholderOptions);
				}
			});
